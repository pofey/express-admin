    ...
    for (var i = 0; i < columns.length; i++) {
        ...
        if (column.oneToMany && column.fk) {
            ...
            if (column.searchview.type == "excludeMany") {
                //excludeMany模式排除选项的value必须为excludeMany喔
                if (value == 1) {
                    statements.push(sql.andValues(link.table, link.parentPk, "not NULL"));
                }if (value == 2) {
                    statements.push(sql.andValues(link.table, link.parentPk, undefined));
                }
            } else {
            ...
            var clause = sql.where(column.searchview, value);
            if (!clause) continue;
