...
var otm = {
    _getRef: function (args, ref, mtmColumn, cb) {
        var pk = sql.as(ref.table, ref.pk, '__pk'),
            text = sql.castConcat(ref.table, ref.columns, '__text');
        var str;
        if (args.id != undefined && ref.joinon) {
            str = query.replace('get-records-join', [pk, text].join(), sql.table(ref.table), sql.joins(ref.table, ref.joinon, args.name, ref.joinon), sql.andValues(args.name, args.config.table.pk, args.id[0]));
        } else {
            str = query.replace('get-records', [pk, text].join(), sql.table(ref.table));
        }
    ...
    get: function (args, cb) {
        ...
        (function loop(i) {
            ...
            if (columns[i].value != undefined) return loop(++i);
            ...
            otm._getRef(args, ref, columns[i], function (err, rows) {
                if (err) return cb(err);
                if (rows != null) columns[i].value = rows;
                loop(++i);
            });
        }(0));
    }
...
var stc = {
    _getSelect: function (options) {
        ...
        for (var i = 0; i < options.length; i++) {
            values.push({__pk: options[i].value, __text: options[i].text, selected: options[i].selected});
        }
        return values;
    },
    ...
    _valueToText: function (data, value) {
    ...
    filterStaticData: function (args, data) {
        var columns = args.config.columns;
        for (var i = 0; i < columns.length; i++) {
            var column = columns[i];
            if (column.staticToColumn == undefined) continue;

            var columnIndex = -1;
            for (var j = 0; j < data.columns.length; j++) {
                if (data.columns[j] == column.verbose) {
                    columnIndex = j;
                    break;
                }
            }
            for (var j = 0; j < data.records.length; j++) {
                ...
                if (data.records[j].pk) index -= 1;
                data.records[j].values[index] = stc._valueToText(args.config.staticData[column.staticToColumn.key], data.records[j].values[index]);
            }
        }
    },
    // loopColumns - add docs
    get: function (args, type) {
        ...
        for (var i = 0; i < columns.length; i++) {
            var control;
            if (type == "edit") {
                control = columns[i].control;
            } else if (type == "list") {
                control = columns[i].searchview;
            } else {
                control = columns[i].control;
            }
            if (control.staticKey == undefined) continue;
            var staticData = args.config.staticData.hasOwnProperty(control.staticKey) ? args.config.staticData[control.staticKey] : undefined;
            if (!(staticData instanceof Array)) continue;

            switch (true) {
                case control.select:
                    columns[i].value = stc._getSelect(staticData);
                    break;
                case control.radio:
                    columns[i].value = stc._getRadio(staticData);
                    break;
            }
        }
    }
