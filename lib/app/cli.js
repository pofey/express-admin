...
cli.promptForDatabase = function (cb) {
    var text = ['Database type: ', 'Database server host: ', 'Database name: ', 'Database user: ', 'Database password: '],
        keys = ['type', 'host', 'database', 'user', 'password'],
        data = {},
        options = {},
        self = this;
    function prompt (text, cb) {
        self.prompt(text, function (input) {
            ...
            if (text == 'Database server host: ') {
                if(input == ''){
                    return cb(input == '' ? 'localhost' : input);
                }
            }
