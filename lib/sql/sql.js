...
exports.andValues = function (table, column, value) {
    ...
    for (var i = 0; i < columns.length; i++) {
        if (values[i] == undefined || values[i] == "NULL") {
            result.push(
                    this.fullName(table, columns[i]) + ' is NULL');
        } else if (values[i] == "not NULL") {
            result.push(
                    this.fullName(table, columns[i]) + ' is not NULL');
        } else {
            result.push(
                    this.fullName(table, columns[i]) + ' = ' + escape(values[i]));
        }
    }
    result = result.join(' AND ');
    return result;
...
// `table1`.`column` like 'value'
// `table1`.`column` like 'value' AND
exports.likeValues = function (table, column, value) {
    var columns = column instanceof Array ? column : [column],
        values = (value instanceof Array)
            ? value
            : (typeof value === 'string' ? value.split(',') : [value]);
    ...
    switch (true) {
        ...
        case control.date || control.time || control.datetime || control.year:
            ...
            var from = value[0] || value[1],
                to = value[1] || value[0];
            clause = ['BETWEEN', escape(from), 'AND', escape(to)];
            break;
